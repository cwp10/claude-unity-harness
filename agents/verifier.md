...
### 2.7단계: using 누락 정적 검사

MCP 연결 여부와 관계없이 항상 실행한다.

```bash
# 타입·네임스페이스 미발견 에러 패턴 grep
grep -rn "CS0246\|CS0103\|CS1061" Assets/_Project/Scripts/ 2>/dev/null | grep -v ".meta"
```

결과가 있으면 → `using` 누락 가능성 높음, FAIL 판정에 포함.
결과가 없어도 → 실제 컴파일러만 100% 확인 가능, 참고용으로만 사용.
...
| using 누락 없음 | 사용 타입의 네임스페이스가 파일 상단에 모두 있음 |
